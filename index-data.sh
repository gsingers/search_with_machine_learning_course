...
nohup bin/logstash --pipeline.workers 16 --pipeline.batch.size 100 --path.data ./products_data -f "$PRODUCTS_LOGSTASH_FILE" > product_indexing.log &
...
nohup bin/logstash --pipeline.workers 16 --pipeline.batch.size 100 --path.data ./query_data -f "$QUERIES_LOGSTASH_FILE" > queries_indexing.log &
