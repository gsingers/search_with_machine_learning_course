...
if [ "$ANNOTATE" == "--annotate" ]; then
  ...
  if [ -f $ANNOTATIONS_JSON_FILE ]; then
    ...
    if [ -f index_products.py ]; then
      ...
      nohup python index_products.py "--synonyms" "--reduced" --index_name "bbuy_annotations" -s "/workspace/datasets/product_data/products" > "/workspace/logs/index_annotations.log" &
      if [ $? -ne 0 ] ; then
