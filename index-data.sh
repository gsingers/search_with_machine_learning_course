...
REPO_DIR="/Users/tholland/search_with_machine_learning_course"
ANNOTATIONS_JSON_FILE="${REPO_DIR}/conf/bbuy_annotations.json"
PRODUCTS_JSON_FILE="${REPO_DIR}/conf/bbuy_products.json"
QUERIES_JSON_FILE="${REPO_DIR}/conf/bbuy_queries.json"
DATASETS_DIR="${REPO_DIR}/workspace/datasets"
PYTHON_LOC="${REPO_DIR}/utilities"
...
if [ "$ANNOTATE" == "--annotate" ]; then
  ...
  if [ -f $ANNOTATIONS_JSON_FILE ]; then
    ...
    if [ -f index_products.py ]; then
      ...
      python index_products.py "--synonyms" "--reduced" --index_name "bbuy_annotations" -s "$DATASETS_DIR/product_data/products" > "$LOGS_DIR/index_annotations.log"
      if [ $? -ne 0 ] ; then
