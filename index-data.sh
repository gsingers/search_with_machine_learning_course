...
nohup bin/logstash --pipeline.workers 16 --pipeline.batch.size 1000 --path.data ./products_data -f "$PRODUCTS_LOGSTASH_FILE" > "$LOGS_DIR/product_indexing.log" &
...
nohup bin/logstash --pipeline.workers 16--pipeline.batch.size 1000 --path.data ./query_data -f "$QUERIES_LOGSTASH_FILE" > "$LOGS_DIR/queries_indexing.log" &
