...
def create_feature_log_query(query, doc_ids, click_prior_query, featureset_name, ltr_store_name, size=200, terms_field="_id"):
    '''
    query: the actual search words, issued by the user, from train.csv, but now stored in the bbuy_queries index
    featureset_name: specified in the conf/ltr_featureset file (product_features)
    ltr_store_name: ?
    doc_ids: the ids of queries from the queries index for a certain query
