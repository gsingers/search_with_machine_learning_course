...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        if response and len(response[hits_field]) > 0:
            hits = response[hits_field][hits_field]
            for hit in hits:
                each_hit_feature = {
                    feature['name']: feature.get('value', 0.0) for feature in hit['fields']['_ltrlog'][0]['log_entry']
                }
                each_hit_feature['doc_id'] = hit['_id']
                each_hit_feature['query_id'] = query_id
                each_hit_feature['sku'] = hit['_id'] #hit.get["sku"]
                feature_results.append(each_hit_feature)
