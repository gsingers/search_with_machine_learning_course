...
def evaluate_test_set(test_data, prior_clicks_df, opensearch, xgb_model_name, ltr_store, index, num_queries=100,
    ...
    no_hand_tuned = []
    no_ltr_hand_tuned = []
    no_results = {"simple": no_simple, "ltr_simple": no_ltr_simple, "hand_tuned": no_hand_tuned,
                  "ltr_hand_tuned": no_ltr_hand_tuned}
    # Build a data frame
    q = []
    sku = []  #
    rank = []
    ...
    for key in query_gb.groups.keys():
        ...
        simple_query_obj = qu.create_simple_baseline(key, click_prior_query, filters=None, size=size, highlight=False,
                                                     include_aggs=False, source=source)
        ...
        # Run hand-tuned
        hand_tuned_query_obj = qu.create_query(key, click_prior_query, filters=None, size=size, highlight=False,
                                               include_aggs=False, source=source)

        __judge_hits(test_skus_for_query, index, key, no_hand_tuned, opensearch, hand_tuned_query_obj, "hand_tuned",
                     results, seen)
        # NOTE: very important, we cannot look at the test set for click weights, but we can look at the train set.

        ltr_simple_query_obj = lu.create_rescore_ltr_query(key, simple_query_obj, click_prior_query, xgb_model_name,
                                                           ltr_store, rescore_size=rescore_size,
                                                           main_query_weight=main_query_weight,
                                                           rescore_query_weight=rescore_query_weight)
        __judge_hits(test_skus_for_query, index, key, no_ltr_simple, opensearch, ltr_simple_query_obj, "ltr_simple",
                     results, seen)
        ltr_hand_query_obj = lu.create_rescore_ltr_query(key, hand_tuned_query_obj, click_prior_query, xgb_model_name,
                                                         ltr_store,
                                                         rescore_size=rescore_size, main_query_weight=main_query_weight,
                                                         rescore_query_weight=rescore_query_weight)
        __judge_hits(test_skus_for_query, index, key, no_ltr_hand_tuned, opensearch, ltr_hand_query_obj,
                     "ltr_hand_tuned", results, seen)
...
def analyze_results(results_df, no_results_df, new_queries_df, opensearch, index, ltr_model_name, ltr_store_name,
                    train_df, test_df, output_dir, precision=10, analyze_explains=False, max_explains=100):
    ...
    print("LTR Hand Tuned MRR is %.3f" % (
        calculate_mrr(results_df, "ltr_hand_tuned", len(no_results_df["ltr_hand_tuned"]))))
    ...
    print("")
    print("Simple p@%s is %.3f" % (
        precision, calculate_precision(results_df, "simple", len(no_results_df["simple"]), precision)))
    print("LTR simple p@%s is %.3f" % (
        precision, calculate_precision(results_df, "ltr_simple", len(no_results_df["ltr_simple"]), precision)))
    print("Hand tuned p@%s is %.3f" % (
        precision, calculate_precision(results_df, "hand_tuned", len(no_results_df["hand_tuned"]), precision)))
    print("LTR hand tuned p@%s is %.3f" % (
        precision, calculate_precision(results_df, "ltr_hand_tuned", len(no_results_df["ltr_hand_tuned"]), precision)))
    ...
    simple_better = simple_join[
        (simple_join["rank_simple"] < simple_join["rank_ltr"]) & (simple_join["found_simple"] == True)]
    ltr_simple_better = simple_join[(simple_join["rank_simple"] > simple_join["rank_ltr"]) & (
            (simple_join["found_simple"] == True) | (simple_join["found_ltr"] == True))]
    simple_ltr_equal = simple_join[
        (simple_join["rank_simple"] == simple_join["rank_ltr"]) & (simple_join["found_simple"] == True)]
    print("Simple better: %s\tLTR_Simple Better: %s\tEqual: %s" % (
        len(simple_better), len(ltr_simple_better), len(simple_ltr_equal)))
    ...
    ltr_ht_better = ht_join[
        (ht_join["rank_ht"] > ht_join["rank_ltr"]) & ((ht_join["found_ht"] == True) | (ht_join["found_ltr"] == True))]
    ...
    if analyze_explains:
        ...
        simple_ltr_explains = compare_explains(simple_better, "ltr_simple", opensearch, index, ltr_model_name,
                                               ltr_store_name, train_gb, max_explains)
        ...
        ht_ltr_explains = compare_explains(ht_better, "ltr_hand_tuned", opensearch, index, ltr_model_name,
                                           ltr_store_name, train_gb, max_explains)
...
def compare_explains(join, type, opensearch, index, ltr_model_name, ltr_store_name, train_gb, max_explains=100):
    ...
    for item in join.itertuples():
        ...
        query_obj, num_shoulds = get_explain_query_for_type(item.query, type, click_prior_query, ltr_model_name,
                                                            ltr_store_name)
