...
def main(**kwargs):
    ...
    create_labeled_queries(kwargs["min_queries"], labeled_data_fpath, kwargs["normalize_queries"])

    # get train test split
    LOGGER.info("Splitting into train and test data")
    data = pd.read_csv(labeled_data_fpath, names=["output", "category"], sep='|', escapechar='\\', quoting=csv.QUOTE_NONE)
    if kwargs["min_queries"] != 1:
        # stratify if possible, only possible where there are 2 or more values per category
        cat_map = {cat:e for e,cat in enumerate(data["category"].unique().tolist())}
        data["cat_id"] = data["category"].map(cat_map)
        train, test = train_test_split(data, test_size=0.1, random_state=42, shuffle=True, stratify=data["cat_id"])
    else:
        train, test = train_test_split(data, test_size=0.1, random_state=42, shuffle=True)
    
    train[["output"]].to_csv(Path(DATA_DIR, "train.txt"), header=False, sep='|', escapechar='\\', quoting=csv.QUOTE_NONE, index=False)
    test[["output"]].to_csv(Path(DATA_DIR, "test.txt"), header=False, sep='|', escapechar='\\', quoting=csv.QUOTE_NONE, index=False)
