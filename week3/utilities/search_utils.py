...
def evaluate_test_set(
    ...
    no_results = {
        "simple": no_simple,
        "ltr_simple": no_ltr_simple,
        "hand_tuned": no_hand_tuned,
    ...
    for key in query_gb.groups.keys():
        ...
        ctr += 1
        test_clicks_for_query = query_gb.get_group(
            key
        ...
        simple_query_obj = qu.create_simple_baseline(
            key,
            click_prior_query,
            filters=None,
            size=size,
        ...
        # Run hand-tuned
        hand_tuned_query_obj = qu.create_query(
            key,
            click_prior_query,
            filters=None,
            size=size,
...
def analyze_results(
    ...
    output_dir,
    precision=10,
    analyze_explains=False,
    max_explains=100,
):
    print(
        "Queries not seen during training: [%s]\n%s\n\n"
    ...
    print(
        "Simple MRR is %.3f"
        % (calculate_mrr(results_df, "simple", len(no_results_df["simple"])))
    )
    print(
    ...
    print(
        ...
        % (
            calculate_mrr(
                results_df, "ltr_hand_tuned", len(no_results_df["ltr_hand_tuned"])
            )
        )
    )
    ...
    print(
        ...
        % (
            precision,
            calculate_precision(
                results_df, "simple", len(no_results_df["simple"]), precision
            ),
        )
    )
    print(
        "LTR simple p@%s is %.3f"
        % (
            precision,
            calculate_precision(
                results_df, "ltr_simple", len(no_results_df["ltr_simple"]), precision
            ),
        )
    )
    print(
        "Hand tuned p@%s is %.3f"
        % (
            precision,
            calculate_precision(
                results_df, "hand_tuned", len(no_results_df["hand_tuned"]), precision
            ),
        )
    )
    print(
        "LTR hand tuned p@%s is %.3f"
        % (
            precision,
            calculate_precision(
                results_df,
                "ltr_hand_tuned",
                len(no_results_df["ltr_hand_tuned"]),
                precision,
            ),
        )
    )
    ...
    simple_join = pd.merge(
        simple_df, ltr_simple_df, on=["query", "sku"], suffixes=("_simple", "_ltr")
    )
    ...
    print(
        "Simple better: %s\tLTR_Simple Better: %s\tEqual: %s"
        % (len(simple_better), len(ltr_simple_better), len(simple_ltr_equal))
    )
    ht_join = pd.merge(
        hand_tuned_df, ltr_hand_tuned_df, on=["query", "sku"], suffixes=("_ht", "_ltr")
    )
    ht_better = ht_join[
    ...
    print(
        ...
        % (len(ht_better), len(ltr_ht_better), len(ht_ltr_equal))
    )
    ...
    if analyze_explains:
        ...
        simple_ltr_explains = compare_explains(
            simple_better,
            "ltr_simple",
            opensearch,
            index,
        ...
        simple_ltr_explains.to_csv(
            "%s/analysis/simple_ltr_explains.csv" % output_dir, index=False
        )
        print("Comparing hand tuned vs LTR explains")
        ht_ltr_explains = compare_explains(
            ht_better,
            "ltr_hand_tuned",
            opensearch,
            index,
        ...
        ht_ltr_explains.to_csv(
            "%s/analysis/hand_tuned_ltr_explains.csv" % output_dir, index=False
        )
...
def compare_explains(
    ...
    for item in join.itertuples():
        ...
        query_obj, num_shoulds = get_explain_query_for_type(
            item.query, type, click_prior_query, ltr_model_name, ltr_store_name
        )
        ...
        if response:
            ...
            if num_shoulds > len(
                details
            ):  # this means not all clauses had explanations
...
def get_explain_query_for_type(
    query, type, click_prior_query, ltr_model_name, ltr_store_name
):
    num_shoulds = 0
    qo = None
    if type == "ltr_simple":
        qo = qu.create_simple_baseline(
            query, click_prior_query, None, include_aggs=False, highlight=False
        )
        qo, num_shoulds = lu.create_sltr_simple_query(
            query, qo, click_prior_query, ltr_model_name, ltr_store_name
        )
    elif type == "ltr_hand_tuned":
        qo = qu.create_query(
            query, click_prior_query, None, include_aggs=False, highlight=False
        )
        qo, num_shoulds = lu.create_sltr_hand_tuned_query(
            query, qo, click_prior_query, ltr_model_name, ltr_store_name
        )
    try:
        qo.pop("size")
...
def lookup_query(
    query, all_clicks_df, opensearch, explain=False, index="bbuy_products", source=None
):
