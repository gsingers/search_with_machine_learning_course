...
if __name__ == "__main__":
    ...
    general.add_argument(
        "--output_dir", default="output", help="the directory to output all files to"
    )
    ...
    dp_group.add_argument(
        "--normalize_json",
        help="A path to a JSON map of LTR feature-normalizer type pairs. If unset, data normalization will not happen.  See week3/conf/normalize_types.json for an example",
    )
    ...
    ltr_group.add_argument(
        "-f", "--featureset", help="The path to the Featureset JSON file to read from"
    )
    ltr_group.add_argument(
        "-n",
        "--featureset_name",
        default="bbuy_main_featureset",
    ...
    xgb_group.add_argument(
        "-x", "--xgb", help="Train an XGB Boost model using the training file given"
    )
    xgb_group.add_argument(
        "-t",
        "--create_xgb_training",
    ...
    xgb_group.add_argument(
        "--xgb_conf",
        default="xgb-conf.json",
        help="Path to XGB parameters JSON dictionary.  See week3/conf/xgb-conf.json",
    )
    xgb_group.add_argument(
        "--xgb_feat_map",
        default="xgb-feat-map.txt",
        help="File name under --output_dir containing the feature map.  Must be set when creating training data.  See week3/conf/xgb-feat-map.txt",
    )
    xgb_group.add_argument(
        "--xgb_rounds",
        default=5,
        type=int,
        help="The number of rounds to train the model on.",
    )
    xgb_group.add_argument(
        "--xgb_model",
        default="xgb_model.model",
    ...
    xgb_group.add_argument(
        "--xgb_model_name", default="ltr_model", help="The name of the model"
    )
    xgb_group.add_argument(
        "--xgb_plot",
        action="store_true",
    ...
    analyze_group.add_argument(
        "--precision", type=int, default=10, help="What precision level to report"
    )
    analyze_group.add_argument(
        "--analyze_explains",
        action="store_true",
    ...
    ltr_store_name = (
        args.ltr_store
    )  # a little extra housekeeping due to the finickiness of the SLTR query
    ...
    if args.split_input:
        ...
        data_prepper.create_splits(
            args.split_input,
            args.split_train,
            args.split_test,
            output_dir,
            args.split_train_rows,
            args.split_test_rows,
            args.verify_file,
        )
    ...
    if args.all_clicks:
        try:
            ...
            all_clicks_df = pd.read_csv(
                args.all_clicks, parse_dates=["click_time", "query_time"]
            )
            all_clicks_df = data_prepper.filter_junk_clicks(
                all_clicks_df, args.verify_file, output_dir
            )
    ...
    if args.upload_ltr_model:
        ...
        ltr.delete_model(
            urljoin(ltr_model_path + "/", "_model/{}".format(args.xgb_model_name)), auth
        )
        featureset_path = urljoin(
            ltr_model_path + "/", "_featureset/{}".format(feat_name)
        )
    ...
    if args.generate_impressions:
        ...
        if (
            args.train_file
        ):  # these should be pre-filtered, assuming we used our splitter, so let's not waste time filtering here
            train_df = pd.read_csv(
                args.train_file, parse_dates=["click_time", "query_time"]
            )
        else:
        ...
        if args.synthesize:
            ...
            impressions_df = impressions_df.sample(
                n=args.generate_num_rows
            ).reset_index(
                drop=True
            )  # shuffle things
    ...
    if args.create_xgb_training and args.impressions_file:
        ...
        if impressions_df is not None:
            ...
            with open(args.featureset) as json_file:
                ...
                features_df = data_prepper.log_features(
                    impressions_df, terms_field=args.ltr_terms_field
                )
                ...
                if args.normalize_json:
                    ...
                    (features_df, aggregations) = data_prepper.normalize_data(
                        features_df, the_feature_set, normalize_type_map
                    )
                ...
                train_features_df = pd.merge(
                    impressions_df, features_df, how="left", on=["query_id", "sku"]
                )
                train_features_df["doc_id"] = train_features_df["sku"]
                # Apply any specified click model.
                train_features_df = cm.apply_click_model(
                    train_features_df, args.click_model, downsample=args.downsample
                )
    ...
    if args.xgb:
        ...
        model = bst.get_dump(
            fmap=("%s/%s" % (output_dir, args.xgb_feat_map)), dump_format="json"
        )
        ...
        ltr.write_opensearch_ltr_model(
            model_name,
            model,
    ...
    if args.xgb_test:
        ...
        no_results_df.to_csv(
            "%s/%s.no_results" % (output_dir, args.xgb_test_output), index=False
        )
        print(
        ...
        new_queries_df = results_df[results_df["new"] == True][
            "query"
        ].drop_duplicates()
        new_queries_df.to_csv(
            "%s/%s.new_queries" % (output_dir, args.xgb_test_output), index=False
        )
    ...
    if args.analyze:
        ...
        test_df = pd.read_csv(
            "%s/test.csv" % output_dir, parse_dates=["click_time", "query_time"]
        )
        train_df = pd.read_csv(
            "%s/%s" % (output_dir, args.train_file),
        ...
        new_queries_df = pd.read_csv(
            "%s/%s.new_queries" % (output_dir, args.xgb_test_output)
        )
        su.analyze_results(
            results_df,
            no_results_df,
            ...
            analyze_explains=args.analyze_explains,
            max_explains=args.max_explains,
        )
