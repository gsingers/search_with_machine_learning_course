...
def create_query(user_query, click_prior_query, filters, sort="_score", sortDir="desc", size=10, source=None, synonym=False):
    query_obj = {
        'size': size,
        "sort": [
            # {sort: {"order": sortDir}}
...
def search(client, user_query, index="bbuy_products", sort=None, sortDir="desc", synonym=False):
...
if __name__ == "__main__":
    ...
    general.add_argument('--synonym', default=True)
    ...
    for line in fileinput.input():
        ...
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, index=index_name, synonym=args.synonym)
