        ...
        "query": {
            "rescore_query": {
                "sltr": {
                    "params": {
                        "keywords": user_query,
                        "clicked_prior_query": click_prior_query
                    },
...
def create_feature_log_query(query, doc_ids, click_prior_query, featureset_name, ltr_store_name, size=200, terms_field="_id"):
    ...
    query_obj = {
        'query': {
            'bool':{
                "filter": [ # Use a filter so that we don't actually score anything
                    ...
                    { # Using the LTR Query to incoporate the LTR Feature set.
                        "sltr": {
        ...
        # Execute the feature logging so that we get weights back for our features.
        "ext": {
            "ltr_log": {
                "log_specs": {
                    "name": "log_entry",
