...
def create_sltr_simple_query(
    user_query, query_obj, click_prior_query, ltr_model_name, ltr_store_name, active_features=None
):
    # Create the base query, use a much bigger window
    # add on the rescore
    sltr = {
...
def create_sltr_hand_tuned_query(
    user_query, query_obj, click_prior_query, ltr_model_name, ltr_store_name, active_features=None
):
    # Create the base query, use a much bigger window
    # add on the rescore
...
def create_feature_log_query(
    query, doc_ids, click_prior_query, featureset_name, ltr_store_name, size=200, terms_field="_id"
):
    ##### Step 3.b:
    query_obj = {
        "query": {
        ...
        "ext": {"ltr_log": {"log_specs": {"name": "log_entry", "named_query": "logged_featureset"}}},
    }
...
def write_training_file(train_data, output_file, feat_map):
    ...
    with open(output_file, "bw") as output:
        for item in train_data.itertuples(index=False):  # skip the first 'index' element from the DF
            ...
            xgb_format = to_xgb_format(
                item.query_id, item.doc_id, item.rank, item.query, item.product_name, item.grade, features
            )
...
def post_featureset(featureset_path, ltr_feature_set, auth, headers={"Content-Type": "application/json"}):
    ...
    resp = requests.post(
        featureset_path, headers=headers, data=json.dumps(ltr_feature_set), auth=auth, verify=False
    )
    return resp
