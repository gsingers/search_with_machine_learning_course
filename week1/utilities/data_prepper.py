        ...
        if response and response['hits']['hits'] and len(response['hits']['hits']) > 0:
            ...
            for hit in hits:
                ...
                for entry in log_entries:
                    feature_results_name_value = feature_results.get(entry.get("name"))
                    if feature_results_name_value is None:
                        feature_results_name_value = []
                        feature_results[entry.get("name")] = feature_results_name_value
                    feature_results_name_value.append(entry.get("value", 0))
