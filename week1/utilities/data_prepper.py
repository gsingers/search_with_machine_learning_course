...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        documents = self.opensearch.search(body=log_query, index=self.index_name)["hits"]["hits"]
        ...
        for document in documents:
            feature_results["doc_id"].append(document["_id"])  # capture the doc id so we can join later
            feature_results["query_id"].append(query_id)
            feature_results["sku"].append(document["_source"]["sku"][0])  
            
            for log_entry_feature_value in document["fields"]["_ltrlog"][0]["log_entry"]:
                name = log_entry_feature_value["name"]
                value = log_entry_feature_value["value"] if "value" in log_entry_feature_value else 0 # value as 0 for the feature not present
