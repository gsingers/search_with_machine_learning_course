...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        for doc in res['hits']['hits']:
            ...
            temp_df = pd.DataFrame().from_dict(features).T
            temp_df.columns = temp_df.iloc[0]
            temp_df = temp_df[1:]
            temp_df['query_id'] = int(query_id)
            temp_df['doc_id'] = docId
            temp_df['sku'] = docId
            frame = frame.append(temp_df)
        ...
        #     features = doc_features.get(str(doc_id),[])
        #     if len(features) > 0:
