...
class DataPrepper:
    ...
    def __init__(
        self,
        opensearch_client,
        featureset_name="bbuy_product_featureset",
        index_name="bbuy_products",
        ltr_store_name="week1",
    ) -> None:
    ...
    def generate_impressions(
        ...
        for key in query_gb.groups.keys():
            ...
            try:
                ...
                if (
                    ...
                    # print(hits)
                    skus_for_query = (
                        prior_clicks_for_query.sku.drop_duplicates()
        ...
        impressions_df = impressions_df[
            (impressions_df["num_impressions"] >= min_impressions)
    ...
    def __log_ltr_query_features(
        ...
        for fsf in feature_set_features:
            feature_results[fsf] = []

        # create query_doc_id:idx map to make sure we're pulling the right features
        # for each doc id
        idx_map = {h["_id"]: e for e, h in enumerate(hits)}
        ...
        for doc_id in query_doc_ids:
            ...
            # get ltr features
            log_entry = hits[idx_map[str(doc_id)]]["fields"]["_ltrlog"][0]["log_entry"]

            features_to_log = {}
            if len(log_entry) > 0:
                for le in log_entry:
                    features_to_log[le["name"]] = le.get("value", 0)
                for fsf in feature_set_features:
                    feature_results[fsf].append(features_to_log.get(fsf, 0))
            else:
                for fsf in feature_set_features:
                    feature_results[fsf].append(features_to_log.get(fsf, 0))
        ...
        return frame.astype(
            {
                ...
                "sku": "int64",
                "name_match": "float",
                "match_phrase_name": "float",
                "match_artist_name": "float",
                "match_short_desc": "float",
                "match_long_desc": "float",
                # "on_sale": "bool",
                "sales_rank_short": "float",
                "sales_rank_long": "float",
                "customer_review_count": "int64",
    ...
    def normalize_data(
        ...
        try:
            ...
            if (
                response
                and response["aggregations"]
                and len(response["aggregations"]) > 0
            ):
                ...
                for agg in agg_fields:
                    ...
                    if norm_type == "min-max":
                        ...
                        max = stats["max"]
                        max_min = max - min
                        ranks_features_df["%s_norm" % agg] = ranks_features_df[
                            agg
                        ].apply(lambda x: (x - min) / max_min)
                    elif norm_type == "std-dev":
                        avg = stats["avg"]
                        std_dev = stats["std_deviation"]
                        ranks_features_df["%s_norm" % agg] = ranks_features_df[
                            agg
                        ].apply(lambda x: (x - avg) / std_dev)
                    # else:
                    # Do nothing for now
            else:
        ...
        return (
            ranks_features_df,
            aggs,
        )  # return out the aggregations, bc we are going to need it to write the model normalizers
