...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        feature_list = ["name_match", "name_match_phrase", "customer_review_average", "customer_review_count", "sales_rank_short_term", 
                        "artist_name_match_phrase", "short_desc_match_phrase", "long_desc_match_phrase", "click_prior"]
        ...
        feature_results["sku"] = []
        for feat in feature_list: # init all features
        ...
        if response and len(response['hits']) > 0:
            ...
            for hit in hits:
                ...
                for feat in feature_list:
                    feature_results[feat].append(self._get_value(log_entry, feat))
        else:
            print("Weirdness. Fix")
    ...
    def _get_value(self, log_entry, feature_name): 
