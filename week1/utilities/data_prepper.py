...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        # IMPLEMENT_START --
        #sprint("IMPLEMENT ME: __log_ltr_query_features: Extract log features out of the LTR:EXT response and place in a data frame")
        response = self.opensearch.search(body=log_query, index=self.index_name)
        
        # Loop over the hits structure returned by running `log_query` and then extract out the features from the response per query_id and doc id.  Also capture and return all query/doc pairs that didn't return features
        # Your structure should look like the data frame below
        if not response or len(response['hits']) == 0:
            raise Exception("bad response from opensearch")
        ...
        for hit in hits:
            ...
            #print(hit['_id'])
            #print(hit['fields']['_ltrlog'][0]['log_entry'][0])
            #print(hit['fields']['_ltrlog'][0]['log_entry'])
            for log_entry in hit['fields']['_ltrlog'][0]['log_entry']:
                feature_name = log_entry['name']
                feature_value = log_entry['value'] if 'value' in log_entry else 0
